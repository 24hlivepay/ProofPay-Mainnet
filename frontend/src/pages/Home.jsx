    ...
    <div className="relative" ref={walletMenuRef}>
      ...
      {walletMenuOpen && (
        <div className="absolute right-0 top-10 z-10 w-full overflow-hidden rounded-xl border border-slate-200 bg-white py-1 text-left shadow-xl">
          ...
          <button onClick={handleDisconnectWallet} className="w-full whitespace-nowrap px-3 py-1.5 text-xs font-semibold text-red-600 hover:bg-red-50">
            Disconnect
          </button>
        </div>
      )}
...
function LiveEscrowOverview({
  ...
  const heroGradient = isTestnetHero
    ...
    : "bg-gradient-to-br from-green-700 via-green-600 to-emerald-700";
  ...
  return (
    <section className={`overflow-hidden rounded-2xl ${heroGradient} p-5 text-white shadow-lg sm:p-6`}>
      ...
      {statsError && <p className={`mt-5 text-sm font-medium ${isTestnetHero ? "text-amber-100" : "text-green-100"}`}>{statsError}</p>}
    </section>
  );
}
...
function StatCard({ label, value }) {
  return (
    <div className="rounded-xl border border-white/20 bg-white/10 p-4 text-white">
      <p className={`text-sm font-medium ${getCurrentNetworkId() === "testnet" ? "text-amber-100" : "text-green-100"}`}>{label}</p>
