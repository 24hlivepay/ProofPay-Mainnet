  ...
  const load = () => api.get("/admin/disputes", { params: { wallet } })
    ...
    .catch((e) => {
      if (e.response?.status === 401 || e.response?.status === 403) {
        navigate("/admin/login", { replace: true, state: { redirectTo: "/admin/disputes" } });
        return;
      }
      setError(e.response?.data?.message || "Unable to load cases.");
    });
  useEffect(() => { load(); }, []);
  async function sendMessage(escrow, text) {
    try { setError("");
      await api.post(`/admin/disputes/${escrow.escrowId}/message`, { wallet, text });
      await load();
    } catch (e) { setError(e.response?.data?.message || "Unable to send message."); throw e; }
  }
...
function ResolvedCase({ escrow, wallet }) {
  ...
  return <article className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
    ...
    {evidence.length > 0 && <><h3 className="mt-4 font-bold">Private evidence</h3><ul className="mt-2 text-sm">{evidence.map((file) => <li key={file.id}><EvidenceLink escrowId={escrow.escrowId} file={file} /></li>)}</ul></>}

    {(dispute.messages || []).length > 0 && <DisputeThread messages={dispute.messages} collapseAfter={3} escrowId={escrow.escrowId} />}
...
function Case({ escrow, wallet, busy, resolve, sendMessage }) {
  const dispute = escrow.dispute; const evidence = [...(dispute.evidence || []), ...(dispute.responses || []).flatMap((response) => response.evidence || []), ...(dispute.messages || []).flatMap((message) => message.evidence || [])];
...
function EvidenceLink({ escrowId, file }) {
  const [failed, setFailed] = useState("");
  async function open() {
    try { setFailed(""); await openEvidence(escrowId, file); }
    catch (openError) { setFailed(openError.message); }
  }
  return <>
    <button type="button" onClick={open} className="text-left text-blue-700 underline">{file.side}: {file.name}</button>
    {failed && <span className="ml-2 text-xs text-red-600">{failed}</span>}
  </>;
}
