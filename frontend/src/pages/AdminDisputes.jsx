...
export default function AdminDisputes() {
  ...
  useEffect(() => { load(); }, []);
  async function sendMessage(escrow, text, files) {
    try { setError("");
      await api.post(`/admin/disputes/${escrow.escrowId}/message`, { wallet, text, files });
      await load();
...
function ResolvedCase({ escrow, wallet }) {
  ...
  return <article className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
    ...
    {(dispute.messages || []).length > 0 && <DisputeThread messages={dispute.messages} collapseAfter={3} escrowId={escrow.escrowId} parties={buildDisputeParties(escrow)} />}
...
function EvidenceLink({ escrowId, file, hideSide = false }) {
  ...
  return <>
    <button type="button" onClick={open} className="text-left text-blue-700 underline">{hideSide ? file.name : `${file.side}: ${file.name}`}</button>
